...
public class Plateau {
	...
	public Robot getRobot(int equipe, int num) {
		for (int i = 0; i < hauteur; i++) {
			for (int j = 0; j < largeur; j++) {
				if (grille[i][j].getRobot(equipe, num) != null)
					return grille[i][j].getRobot(equipe, num);
			}
		}
		return null;
	}
	...
	private void placerObstacles() {
		...
		while (obstacles != ((Plateau.hauteur * Plateau.largeur) * Plateau.tauxObstacles)) {
			...
			if (!grille[x][y].estObstacle() && (grille[x][y].estBase() == 0)) {
				grille[x][y].placerObstacle();
				obstacles++;
			}
		}
	}
	...
	public boolean parcourtValide(boolean[][] p, int x, int y, int test) {
		try {
			if (p[hauteur - 1][largeur - 1])
				return true;
			// DROITE
			else if ((y != (largeur - 1)) && !grille[x][y + 1].estObstacle()
					&& !p[x][y + 1]) {
				p[x][y + 1] = true;
				return parcourtValide(p, x, y + 1, test++);
			}
			// BAS
			else if ((x != (hauteur - 1)) && !grille[x + 1][y].estObstacle()
					&& !p[x + 1][y]) {
				p[x + 1][y] = true;
				return parcourtValide(p, x + 1, y, test++);
			}
			// GAUCHE
			else if ((y != 0) && !grille[x][y - 1].estObstacle()
					&& !p[x][y - 1]) {
				p[x][y - 1] = true;
				return parcourtValide(p, x, y - 1, test++);
			}
	...
	public String toString(int equipe, int num) {
		...
		for (int i = 0; i < hauteur; i++) {
			s = s + "\n|";
			for (int j = 0; j < largeur; j++) {
				if (grille[i][j].estMine() != 0) {
					if (equipe == 0) {
						s = s + " X |";
					} else if (grille[i][j].estMine() == equipe) {
						s = s + " X |";
					}

				} else if (grille[i][j].getContenu() instanceof Tireur) {
					if (grille[i][j].getContenu().getEquipe() == 1) {
						s = s + " T |";
					} else {
						s = s + " t |";
					}
				} else if (grille[i][j].getContenu() instanceof Piegeur) {
					if (grille[i][j].getContenu().getEquipe() == 1) {
						s = s + " P |";
					} else {
						s = s + " p |";
					}
				} else if (grille[i][j].getContenu() instanceof Char) {
					if (grille[i][j].getContenu().getEquipe() == 1) {
						s = s + " C |";
					} else {
						s = s + " c |";
					}
				} else if (grille[i][j] instanceof Base) {
